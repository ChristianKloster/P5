...
apiservice = 'http://api.gofact.net/api/external/sale/retailerid/'
#{retailerid}/date/{date}
...
# Get sales from all retailers and save in rpt file 'Sales_{date}.rpt'
def main():
	year = '2017'
	...
	for month in ['11']:
		...
		for day in range(1,32):
			...
			for ret in retailers:
				data = get_sales(ret, date)
				row = pd.DataFrame(data)
				df = df.append(row)

		df.to_csv('Sales_' + d + '.rpt' ,index=False ,sep=';')


# Make request for stock data. Returns list of dicts '[{productId:val, retailerId:val, stockcount:val },...]'
def get_sales(retailerID, date):
	s = make_request_string(retailerID, date)
...
def make_request_string(retailer_ID, date):
	return apiservice + str(retailer_ID) + '/date/' + date
